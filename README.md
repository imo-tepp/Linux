...
## Linux Processes
When a program is being executed, they are in *Process* of being executed.  Multples Processes can be started via by opening an application, where as a single process is created when a terminal command is executed. Each process has *PID (Process Identifcation number)* or *PPID (Parent Process Identifcation number)* There are different types of process that runs on linux, for example:
...
- Daemon processes: are processess that begin at the system startup and continuously running in the background.
- The `ps` command output a static view of all processess
- The `top` command display the real-time list of all running processes.
- `htop` - same as top command except it's more user friendly.  
...
- `pidof`: finds the ID of a process
- `nohup`: *No Hang Up* prevents the process from being terminated when exiting the terminal
- `nice` : changes the priority of the processes.
- System(), fork() and exec() are all functions that creates a new process. System() is the simpler one of the three but comes with security risk, whereas the other 2 are more complicated to use but offer better flexilibty and speed with security.

### Zombie and Orphan Process
- Zombie process is when a process is killed but the *ps* listing still show the killed process with a Z state. It normally occurs when a parent process starts a child process but don't pick up the child's exit code. The process becomes a zombie process as it is dead but still exist in the machine.
- Orphan process is when a parent process initate a child process, but the parent process finishes. Thus, leaving the child process (orphaned) to keep running indefinitely.

### States of Processes
- *Running* - a process which is running or waiting for a resource to free up and run it.
- *Waiting* - a process that is waiting for the output of another process or an event to initiate it.
- *Stopped* - process that's been stopped by another process or user.
- *Zombie* - dead process but still visible in the process table.
...
- `nslookup` Name Server LookUp: get mapping information about the domain name and IP address.
- `iperf` : analyze and measure the network performance between the hosts.
- `ss`: displays all the sockets that are on the computer.
- `telnet`: connect to the hosts via a telnet protocol , successfully connecting means the connectivity between the hosts are fine.
- `dig` (Domain Information Groper): perfroms DNS lookups and displays the result from the name servers.
- `ssh`: secure connection to remote hosts by encrypting the communication between the host.
